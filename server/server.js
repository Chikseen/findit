...
io.on("connection", (socket) => {
  ...
  socket.on("checkSID", async (data) => {
    console.log("Check if SID exits");
    console.log("SIDs", sessionIds);
    console.log("Check With", data.SID);

    if (sessionIds.includes(parseInt(data.SID))) {
      console.log("SID Exits");
      socket.emit("respSID", { status: "valid" });
    } else {
      socket.emit("respSID", { status: "unvalid" });
      console.log("SID not exits");
    }
  });

  socket.on("destroySession", async (data) => {
    console.log("Destroy Session", data.SID);
